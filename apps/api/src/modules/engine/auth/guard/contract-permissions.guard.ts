...
export const ContractPermission = (paramName: string, options?: ContractPermissionOptions) => {
  return (target: any, propertyKey: string, descriptor: PropertyDescriptor) => {
    Reflect.defineMetadata(CONTRACT_PERMISSION_KEY, {
      paramName,
      options: {
        mode: 'single',
        required: true,
...
export class ContractPermissionsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!permissionConfig) {
      this.logger.debug(`Nenhuma verificação de permissão configurada para ${context.getHandler().name}`);
      return true;
    }
    ...
    try {
      ...
      const contractData = this.extractContractData(request, paramName, options);

      if (!contractData && options.required) {
        this.logger.warn(`Parâmetro de contrato '${paramName}' não encontrado para usuário ${userId}`);
        throw new BadRequestException(`Parâmetro '${paramName}' é obrigatório`);
      }

      if (!contractData) {
        this.logger.debug(`Parâmetro de contrato '${paramName}' não fornecido, permitindo acesso`);
        return true;
      }

      // Verificar permissões baseado no modo
      const hasPermission = await this.checkPermission(userId, contractData, options);

      if (!hasPermission) {
        this.logger.warn(`Acesso NEGADO: usuário ${userId} não tem permissão para contrato(s) ${JSON.stringify(contractData)}`);
        throw new ForbiddenException('Você não tem permissão para acessar este contrato');
      }

      this.logger.log(`Acesso CONCEDIDO: usuário ${userId} tem permissão para contrato(s) ${JSON.stringify(contractData)}`);
      return true;
      ...
      this.logger.error(`Erro ao verificar permissões para usuário ${userId}:`, error);
   ...
   */
  private extractContractData(request: any, paramName: string, options: ContractPermissionOptions): number | number[] | null {
    ...
    if (contractData) {
      if (Array.isArray(contractData)) {
        return contractData.map(id => parseInt(id, 10)).filter(id => !isNaN(id));
      }
   ...
   */
  private async checkPermission(userId: number, contractData: number | number[], options: ContractPermissionOptions): Promise<boolean> {
    const { mode = 'single' } = options;

    switch (mode) {
      case 'single':
        if (typeof contractData !== 'number') {
          this.logger.warn(`Modo 'single' espera número, recebido: ${typeof contractData}`);
          return false;
        }
        return await this.contractPermissionsService.hasContractPermission(userId, contractData);

      case 'any':
        if (!Array.isArray(contractData)) {
          this.logger.warn(`Modo 'any' espera array, recebido: ${typeof contractData}`);
          return false;
        }
        return await this.contractPermissionsService.hasAnyContractPermission(userId, contractData);

      case 'all':
        if (!Array.isArray(contractData)) {
          this.logger.warn(`Modo 'all' espera array, recebido: ${typeof contractData}`);
          return false;
        }
        // Para modo 'all', verificar se tem permissão para todos os contratos
        const permissions = await Promise.all(
          contractData.map(contractId =>
            this.contractPermissionsService.hasContractPermission(userId, contractId)
          )
        );
        return permissions.every(hasPermission => hasPermission);

      default:
        this.logger.warn(`Modo de verificação desconhecido: ${mode}`);
        return false;
    }
  }
}

/**
 * DTO para sincronização da relação Checklist -> Tipo de Equipe
 *
 * Define a estrutura de dados para sincronização
 * da relação entre checklists e tipos de equipe.
 */
 ...
 * DTO para sincronização da relação Checklist -> Tipo de Equipe
 */
export class ChecklistTipoEquipeRelacaoSyncDto {
  @ApiProperty({
    description: 'ID da relação entre checklist e tipo de equipe',
    example: 6,
  })
  @IsNumber()
  @IsPositive()
  ...
  @ApiProperty({ description: 'ID do tipo de equipe vinculado', example: 1 })
  @IsNumber()
  @IsPositive()
  tipoEquipeId: number;

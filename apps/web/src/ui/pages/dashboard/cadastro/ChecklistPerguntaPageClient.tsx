...
import ChecklistPerguntaForm from '@/ui/pages/dashboard/cadastro/checklist-pergunta/form';

interface ChecklistPerguntaPageClientProps {
  initialData?: PaginatedResult<ChecklistPergunta>;
}

export default function ChecklistPerguntaPageClient({
  initialData,
}: ChecklistPerguntaPageClientProps) {
  ...
  const columns = useTableColumnsWithActions<ChecklistPergunta>(
    ...
    ],
    {
      ...
      onDelete: (item) =>
        controller
          .exec(
            () => deleteChecklistPergunta({ id: item.id }),

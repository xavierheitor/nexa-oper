...
interface EquipePageClientProps {
  initialEquipes?: PaginatedResult<EquipeWithBase>;
  initialBases?: Base[];
  initialContratos?: Contrato[];
  initialTiposEquipe?: TipoEquipe[];
}
...
export default function EquipePageClient({
  ...
  const { data: tiposEquipe } = useEntityData<TipoEquipe>({
    key: 'tipos-equipe-lote',
    fetcherAction: unwrapFetcher(listTiposEquipe),
    initialData: initialTiposEquipe,
    initialParams: { page: 1, pageSize: 100, orderBy: 'nome', orderDir: 'asc' },
    paginationEnabled: false,
  });

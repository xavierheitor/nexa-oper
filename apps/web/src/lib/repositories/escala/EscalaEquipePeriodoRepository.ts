...
export type EscalaEquipePeriodoUpdateInput =
  Partial<EscalaEquipePeriodoCreateInput> & {
    id: number;
    status?: StatusEscalaEquipePeriodo;
    versao?: number;
  };
...
> {
  ...
  protected async findMany(
    ...
    return prisma.escalaEquipePeriodo.findMany({
      ...
      take,
      include: (include ||
        this.getDefaultInclude()) as Prisma.EscalaEquipePeriodoInclude,
    });
  }
  ...
  async list(params: EscalaEquipePeriodoFilter) {
    ...
    const where: Prisma.EscalaEquipePeriodoWhereInput = {
      ...
      // Filtros de relacionamento com Equipe (Base e Tipo) otimizados
      ...((baseId !== undefined || tipoEquipeId) && {
        equipe: {
          deletedAt: null,

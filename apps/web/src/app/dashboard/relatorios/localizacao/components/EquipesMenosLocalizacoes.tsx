...
export default function EquipesMenosLocalizacoes({
  ...
  const { data: dados = [], loading, error, refetch } = useDataFetch<EquipeLocalizacaoStats[]>(
    async () => {
      ...
      const result = await getEquipesMenosLocalizacoes(filtros);

      if (result.success && result.data) {
        return result.data;
      }
      throw new Error('Erro ao carregar dados de equipes com menos localizações');
    },
    [filtros]
  );
  ...
  const columns: ColumnsType<EquipeLocalizacaoStats> = [
    ...
    },
    {
      ...
      key: 'ultimaCaptura',
      render: (value: Date | null) => formatarData(value),
      sorter: (a, b) => {
    ...
    },
    {
      ...
      render: (value: number | null) => {
        if (value === null) return <Tag color="default">N/A</Tag>;
        const horas = Math.floor((value || 0) / 60);
        const dias = Math.floor(horas / 24);
        if (dias > 0) {
          return <Tag color="red">{formatarTempoSemCaptura(value)}</Tag>;
        }
        if (horas > 0) {
          return <Tag color="orange">{formatarTempoSemCaptura(value)}</Tag>;
        }
